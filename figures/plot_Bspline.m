...
%% U_MAX
subaxis(3,3,2, 'Spacing', 0.03, 'Padding', 0, 'Margin', 0);
axis off
hold on;
...
%% Time Horizon
subaxis(3,3,5, 'Spacing', 0.03, 'Padding', 0, 'Margin', 0);
axis off
hold on;
patch([b_x(2) b_x(1) -b_x(2) -b_x(1)],[b_x(4) -b_x(3) -b_x(4) b_x(3)],'k','facealpha',0.1)
axis([-b_x(2)-0.1 b_x(1)+0.1 -b_x(4)-0.1 b_x(3)+0.1]);
...
% this uses xbar as ref

subaxis(3,3,8, 'Spacing', 0.03, 'Padding', 0, 'Margin', 0);
axis off
hold on;
...
for j = 1:size(X0,2)
    ...
    for i = 1:steps
        x0_ = x1;
        A = Df_func(x0_(1),x0_(2));
        if i == 1
            x1 = expm(A*(dt/steps)/2)*x0_;
        else
            x1 = expm(A*dt/steps)*x0_;
        end
        xbar = [xbar x1];
    end
    f_xbar = f(xbar')';
...
%     steps_ = STEPS(j);
%     xbar = repmat(XBAR,1,steps_);
%     f_xbar = repmat(f_xbar,1,steps_);
%     g_xbar = repmat(g_xbar,1,steps_);
%     tic
%     [Q,Q_combined] = Bezier.Q(steps_, 3);
%     [F, G] = Bezier.F_G(A_x, b_x, H, xbar, f_xbar, g_xbar, gamma,Q,Lg,Lf,e_bar,K,u_max);
%
%     T(j) = toc;
%
%     A_in = F*D_nT;
...
%         ind = convhull(Vert);
%         Vert = Vert(ind,:);
%         patch(Vert(:,1),Vert(:,2),c1+j/length(STEPS)*(c2-c1),'facealpha',0.01,'edgecolor',c1_f+j/length(STEPS)*(c2_f-c1_f),'linewidth',2);
% end
