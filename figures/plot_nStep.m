...
%% U_MAX
subaxis(3,3,3, 'Spacing', 0.03, 'Padding', 0, 'Margin', 0);
axis off
hold on;
...
for j = 1:length(U_MAX)
    ...
    for k = 1:steps
        ...
        for i = 1:size(Vert_,1)
            IC = Vert_(i,:)'*0.99; % Does not like sampling on boundary
            ...
            % Forward
            Vert_f = cddmex('extreme',struct('A',[D_vec(1:2,:); A],'B',[IC;b],'lin',1:2));
    ...
    patch(Vert(:,1),Vert(:,2),c1+j/length(U_MAX)*(c2-c1),'facealpha',0.01,'edgecolor',c1_f+j/length(U_MAX)*(c2_f-c1_f),'linewidth',2);
end

%% Time Horizon
subaxis(3,3,6, 'Spacing', 0.03, 'Padding', 0, 'Margin', 0);
axis off
hold on;
patch([b_x(2) b_x(1) -b_x(2) -b_x(1)],[b_x(4) -b_x(3) -b_x(4) b_x(3)],'k','facealpha',0.1)
axis([-b_x(2)-0.1 b_x(1)+0.1 -b_x(4)-0.1 b_x(3)+0.1]);
...
for j = 1:length(DT)
    ...
    patch(Vert(:,1),Vert(:,2),c1+j/length(DT)*(c2-c1),'facealpha',0.01,'edgecolor',c1_f+j/length(DT)*(c2_f-c1_f),'linewidth',2);
end

%% Initial Condition
subaxis(3,3,9, 'Spacing', 0.03, 'Padding', 0, 'Margin', 0);
axis off
hold on;
patch([b_x(2) b_x(1) -b_x(2) -b_x(1)],[b_x(4) -b_x(3) -b_x(4) b_x(3)],'k','facealpha',0.1)
axis([-b_x(2)-0.1 b_x(1)+0.1 -b_x(4)-0.1 b_x(3)+0.1]);
...
for j = 1:size(X0,2)
    ...
    toc
    Vert = Vert_;
    patch(Vert(:,1),Vert(:,2),c1+j/num*(c2-c1),'facealpha',0.01,'edgecolor',c1_f+j/num*(c2_f-c1_f),'linewidth',2);
